  ...
  <div className='h-auto mt-2 flex flex-col gap-8'>
    {chatHistory.length > 0 && chatHistory.map((chat) => {
      if (chat?.role === "assistant") {
        ...
        <div key={chat?.id}>
           {
          chat?.content !=='' &&  <div key={chat?.id} className="flex-row justify-start w-1/2 lg:w-2/3 ml-auto h-auto rounded-2xl bg-emerald-100 rounded-br-none p-3">
          {chat?.content}
        </div>
         }
        </div>
      ...
      <div key={chat?.id}>
         {
          chat?.content !=='' &&  <div key={chat?.id} className="flex-row justify-end w-1/2 lg:w-2/3 mr-auto h-auto rounded-2xl bg-emerald-100 rounded-bl-none p-3">
          {chat?.content}
        </div> 
         }
      </div>
